...
* [시나리오 E — Qwen 판정 갈아치우기](#시나리오-e--qwen-판정-갈아치우기)
...
## 시나리오 E — Qwen 판정 갈아치우기

시나리오 C의 VERIFY 쪽 짝이다. **Qwen이 뭐라고 판정했는지 화면에서 먼저 읽고**
`내 판정: keep / revert / done` 을 누른다.

* `verified_by` 가 `operator` 로 찍힌다.
* Qwen의 원래 판정은 `verify.json` 의 `model_decision` 에 기록으로만 남는다.
* 같은 줄에 이유를 함께 적으면 그것이 다음 PLAN으로 실려 간다.
* 별도 옵션이 필요 없다 — `--ui` 만 켜면 된다.

![VERIFY 화면](images/04-verify-with-region.png)

Qwen 판정은 그대로 두고 의견만 붙이려면 `참고로 첨부 (판정 유지)` 를 쓴다
(시나리오 B와 같다).
...
Qwen은 어느 쪽이든 두 단계 모두 판정한다. 이 스위치는 **루프가 나를 위해
멈추는지**만 정한다.
